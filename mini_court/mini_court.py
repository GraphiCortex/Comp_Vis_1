...
sys.path.append('../')
import constants
from utils import convert_pixel_distance_to_meters, convert_meters_to_pixels_distance, get_foot_position, get_closest_keypoint_index, get_height_of_bbox, measure_xy_dist, get_center_of_bbox, measure_distance
...
class MiniCourt():
    ...
    def get_mini_court_coordinates(self, object_pos, closest_keypoint,
                                   closest_keypoint_index, player_height_in_pix,
                                   player_height_in_m):
        dist_from_keypoint_x_pixels, dist_from_keypoint_y_pixels = measure_xy_dist(object_pos, closest_keypoint)
        #Convert pixel dist to meters
        dist_from_keypoint_x_in_m = convert_pixel_distance_to_meters(dist_from_keypoint_x_pixels,
                                                                     player_height_in_m,
                                                                     player_height_in_pix)

        dist_from_keypoint_y_in_m = convert_pixel_distance_to_meters(dist_from_keypoint_y_pixels,
                                                                     player_height_in_m,
                                                                     player_height_in_pix)
        
        # Convert to mini court coordinates
        mini_court_x_dist_in_pix = self.convert_meters_to_pixels(dist_from_keypoint_x_in_m)
        mini_court_y_dist_in_pix = self.convert_meters_to_pixels(dist_from_keypoint_y_in_m) 
        ...
        mini_court_player_pos = (
            closest_mini_court_keypoint[0] + mini_court_x_dist_in_pix,
            closest_mini_court_keypoint[1] + mini_court_y_dist_in_pix
        )
        
        return mini_court_player_pos
        
    def convert_bouding_boxes_to_mini_court_coordinates(self, player_boxes, ball_boxes, original_court_keypoints):
        player_heights = {
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ...
            for player_id, bbox in player_bbox.items():
                ...
                output_player_bboxes_dict[player_id] = mini_court_player_pos
